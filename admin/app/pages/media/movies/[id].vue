<template>
  <GalssPanel>
    <div class="navigation-bar">
      <NuxtLink to="/media/movies" class="btn-back">← Назад</NuxtLink>
    </div>
    ...
    <BaseForm class="user-form-container">
      ...
      <div v-else class="form-content">
        <div class="form-header">
          <h1 class="page-title">Фильм №{{ movieId }}</h1>
        </div>
        ...
        <div class="form-section">
          <button
            ...
            :disabled="isProcessing"
          >
            {{ isProcessing ? 'Удаление...' : 'Удалить фильм' }}
          </button>
        </div>
      </div>
    </BaseForm>
...
onMounted(async () => {
  fetchCategories()
  try {
    const movie = await mediaService.getById(movieId)
    form.title = movie.title
    form.description = movie.description
    form.category_ids = (movie.categories || [])
      .map((c: any) => c?.id)
      .filter((id: any): id is number => typeof id === 'number')
  } catch (e: any) {
    error.value = e.message || 'Ошибка загрузки фильма'
  } finally {
    pending.value = false
  }
})
...
const handleUpdate = async () => {
  try {
    ...
    await mediaService.update(movieId, {
      title: form.title,
      description: form.description,
      category_ids: form.category_ids
    })

    useToastify('Фильм обновлён', {
      type: 'success',
      autoClose: 3000,
      theme: 'auto'
    })
...
const handleDelete = async () => {
  ...
  try {
    isProcessing.value = true
    await mediaService.delete(movieId)
    router.push('/movies')
  } catch (err: any) {
    useToastify(`Ошибка ${err.status || ''}`, {
      type: 'error',
